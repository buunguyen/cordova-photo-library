...
PhotoLibrary.fromImage = function (img, successCallback, failureCallback) {
  if (!img.complete) {
    return (failureCallback || noop)(new Error('Image has not been loaded'))
  }
...
PhotoLibrary.fromBase64 = function (base64Str, successCallback, failureCallback) {
  return cordova.exec(successCallback || noop, failureCallback || noop, 'PhotoLibrary', 'fromBase64', [base64Str]);
}

PhotoLibrary.fromUrl = function (imageUrl, successCallback, failureCallback) {
  return cordova.exec(successCallback || noop, failureCallback || noop, 'PhotoLibrary', 'fromUrl', [imageUrl]);
}
